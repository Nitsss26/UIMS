...
const navItems: NavItem[] = [
    { label: 'Dashboard', href: '/fees', icon: LayoutGrid, color: 'text-blue-600' },
    { label: 'Fee Collection', href: '/fees/collection', icon: IndianRupee, color: 'text-emerald-500' },
    { label: 'Fee Structure', href: '/fees/structure', icon: Wallet, color: 'text-sky-500' },
    { label: 'Receipts & Invoices', href: '/fees/receipts', icon: Receipt, color: 'text-amber-500' },
    { label: 'Student Dues', href: '/fees/dues', icon: Users, color: 'text-rose-500' },
    { label: 'Scholarships', href: '/fees/scholarships', icon: Award, color: 'text-purple-500' },
    { label: 'Defaulters List', href: '/fees/defaulters', icon: AlertCircle, color: 'text-red-500' },
    { label: 'Reports', href: '/fees/reports', icon: FileText, color: 'text-slate-600' },
    { label: 'Settings', href: '/fees/settings', icon: Settings, color: 'text-slate-500' },
];
...
export default function FeeAccountantLayout() {
    ...
    return (
        <div className="min-h-screen bg-slate-50">
            ...
            <div className="lg:ml-64">
                            ...
                            <DropdownMenu>
                                    ...
                                    <Button variant="ghost" className="flex items-center gap-2">
                                        <Avatar className="h-8 w-8">
                                            ...
                                            <AvatarFallback className="bg-blue-100 text-blue-600 text-sm font-medium">
                                                {auth.user?.name?.charAt(0) || 'F'}
                                            </AvatarFallback>
                                        </Avatar>
                                        <div className="hidden sm:flex flex-col items-start">
                                            <span className="text-sm font-medium text-slate-700">{auth.user?.name || 'Fee Accountant'}</span>
                                            <span className="text-xs text-slate-500 capitalize">{auth.role || 'Accounts'}</span>
                                        </div>
                                ...
                                <DropdownMenuContent align="end" className="w-56">
                                    ...
                                    <DropdownMenuItem asChild>
                                        <Link to="/fees/profile" className="flex items-center gap-2 cursor-pointer">
                                            <Users className="w-4 h-4" />
                                            Profile
                                        </Link>
                                    </DropdownMenuItem>
                                    <DropdownMenuItem asChild>
                                        <Link to="/fees/settings" className="flex items-center gap-2 cursor-pointer">
                ...
                {/* Page Content */}
                <div className="p-4 lg:p-6 text-slate-900 font-medium">
                    <Outlet />
